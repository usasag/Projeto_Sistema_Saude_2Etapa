...
int removeService (struct RemoveService service) 
{
    if (service.totalServices == 0) {
        printf("No service available for removal.\n");
        return service.totalServices;
    }

    int ServiceCode;
    printf("Enter the Service Code for removal: ");
    scanf("%d", &ServiceCode);

    int indiceAtendimento = -1;
    for (int i = 0; i < service.totalServices; i++) {
        if (service.totalServices[i] == ServiceCode) {
            indiceAtendimento = i;
            break;
        }
    }
    ...
    if (codigoConfirmacao != ServiceCode) {
    ...
    for (int i = indiceAtendimento; i < service.totalServices - 1; i++) {
        service.totalServices[i] = service.totalServices[i + 1];
        strcpy(service.status[i], service.status[i + 1]);
    }

    printf("Atendimento removido com sucesso.\n");
    return service.totalServices - 1; // Retorna o número atualizado de atendimentos
}
        ...
        )
{
    ...
    int proximoServiceCode = totalAtendimentos + 1;
    codigosAtendimentos[totalAtendimentos] = proximoServiceCode;
...
void editarAtendimento(
    ...
    int indiceAtendimento = -1;
    for (int i = 0; i < totalAtendimentos; i++) {
        if (codigosAtendimentos[i] == ServiceCode) {
            indiceAtendimento = i;
            break;
        }
    }
